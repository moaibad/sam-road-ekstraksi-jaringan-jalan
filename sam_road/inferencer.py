...
parser.add_argument(
    "--gamma", default='1', help="Name of the output dir, if not specified will use timestamp"
)
...
def infer_one_img(net, img, config):
    ...
    # Save before gamma
    cv2.imwrite(os.path.join("D:\Kuliah\\bismillah-yudis-1\Tools\\ape\save\\mask", 'result_no_gamma.png'), fused_road_mask)
...
if __name__ == "__main__":
    ...
    output_dir = 'D:\Kuliah\\bismillah-yudis-1\Tools\\ape\save'
    ...
    # [H, W, C] RGB
    img = read_rgb_img('D:\Kuliah\\bismillah-yudis-1\Tools\\ape\input.png')
    ...
    viz_nms = triage.visualize_image_and_nodes_only(viz_img, pred_nodes/img_size, viz_img.shape[0])
    cv2.imwrite(os.path.join(viz_save_dir, f'{img_id}_points.png'), viz_nms)
